...
vars = @variables begin
    ...
    (Nᵢᵍ(t))[1:Nc], [description = "Molar holdup of each component in gas phase (mol)", guess = [0.0, 0.0, 10.0]]
    (xᵢ(t))[1:Nc], [description = "Molar fraction in liquid phase (-)", guess = [0.0, 0.0, 1.0]]
    (yᵢ(t))[1:Nc], [description = "Molar fraction in gas phase (-)", guess = [0.0, 0.0, 1.0]]
    ...
    Vᴸ(t), [description = "Volume of the liquid phase (m³)", guess = 0.5*Vtot_, irreducible = true]
    Vᵍ(t), [description = "Volume of the gas phase (m³)", guess = 0.5*Vtot_, irreducible = true]
    ρᴸ(t), [description = "Molar Density of gas phase (mol/m³)", guess = molar_density(model, 1.5*101325.0, 377.0, [0.0, 0.0, 0.001], phase = :liquid)]
    ρᵍ(t), [description = "Molar Density of liquid phase (mol/m³)"]
    Nᴸ(t), [description = "Total molar holdup in the liquid phase (mol)", guess = 0.0001] 
...
fugacities_g = [ϕᵢᵍ[i] ~ fugacity_coefficient(model, P, T, yᵢ, phase = "vapor")[i] for i in 1:Nc]

fugacities_l = [ϕᵢᴸ[i] ~ fugacity_coefficient(model, P, T, xᵢ, phase = "liquid")[i] for i in 1:Nc]
