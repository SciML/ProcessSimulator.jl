@component function ThreePortDrum(; substances,
    ...
    vars = @variables begin
        ...
        Vᴸ(t),
        [description = "Volume of the liquid phase (m³)",
            guess = 0.5 * Vtot_, irreducible = true]
        Vᵍ(t),
        [description = "Volume of the gas phase (m³)",
            guess = 0.5 * Vtot_, irreducible = true]
        ρᴸ(t),
    ...
    component_balance = [D(Nᵢ[i]) ~ In.F * In.z₁[i] - Fᴸ_outflow * xᵢ[i] -
                                    Fᵍ_outflow * yᵢ[i] for i in 1:Nc]

    energy_balance = [D(U) ~ In.F * In.H - Fᴸ_outflow * hᴸ_outflow -
                             Fᵍ_outflow * hᵍ_outflow + Q̇]
    ...
    non_smoothness = [_0_Nᵍ ~ Nᵍ / (Nᵍ + Nᴸ)
                      _0_Nᴸ ~ Nᵍ / (Nᵍ + Nᴸ) - 1.0
                      0.0 ~ _0_Nᵍ + _0_Nᴸ + (sum(collect(xᵢ)) - sum(collect(yᵢ))) -
