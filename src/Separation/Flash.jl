@component function ThreePortDrum(;
        substances,
    ...
    component_balance = [
        D(Nᵢ[i]) ~ In.F * In.z₁[i] - Fᴸ_outflow * xᵢ[i] - Fᵍ_outflow * yᵢ[i]
            for i in 1:Nc
    ]
    ...
    liquid_gas_enthalpy = [
        hᴸ_outflow ~ enthalpy(model, P, T, xᵢ, phase = "liquid")
        hᵍ_outflow ~ enthalpy(model, P, T, yᵢ, phase = "vapor")
    ]

    liquid_gas_densities = [
        ρᴸ ~ molar_density(model, P, T, Nᵢᴸ, phase = "liquid")
    ...
    fugacities_g = [
        ϕᵢᵍ[i] ~ fugacity_coefficient(model, P, T, yᵢ, phase = "vapor")[i]
            for i in 1:Nc
    ]

    fugacities_l = [
        ϕᵢᴸ[i] ~ fugacity_coefficient(model, P, T, xᵢ, phase = "liquid")[i]
            for i in 1:Nc
    ]
    ...
    non_smoothness = [
        ...
        0.0 ~
            ...
            max(_0_Nᵍ, _0_Nᴸ, sum(collect(xᵢ)) - sum(collect(yᵢ)))
    ...
    eqs = [
        liquid_gas_holdups...; xy_fractions...; component_holdups...;
