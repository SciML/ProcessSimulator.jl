...
vars = @variables begin
    ...
    P_out(t), [description = "Pressure at the outlet stream level (Pa)", guess = 101325.]
...
globalEnthalpy_eq = [H ~ enthalpy(model, P_out, T, Nᵢ, phase = "unknown") + sum(scalarize(ΔH₀f.*Nᵢ)), fᵢ ~ fugacity_coefficient(model, P_out, T, Nᵢ, phase = "liquid")[1]] #fᵢ ~ fugacity_coefficient(model, P_out, T, Nᵢ, "liquid")[1]
molar_mass = [MW ~ sum(scalarize(MWs.*Nᵢ))/N*gramsToKilograms]


eqs = [reaction_rate...; overall_reaction_rate...; atm_pressure...; inletenthalpy...; inlettemperature_eqs...; inletmolarflow_eqs...; out_conn...;
out_conn_phases...; component_balance...; energy_balance...; jacket_energy_balance...; mass_volume_eq...; mol_holdup...; mol_to_concentration...; 
height_to_volume...; volumetricflow_to_molarflow...; volumetricflow_to_massflow...;
