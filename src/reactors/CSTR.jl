@component function CSTR(
        ms::MaterialSource; name, i_reacts = 1:length(ms.reaction), flowtype = "")
    # Subsystems
    ...
    eqs = Equation[
        ...
        [cv.ΔnR[i] ~ reac.r(cv.p, cv.T, collect(cv.xᵢ[1, :])) * reac.ν[i] * cv.n
         for i in 1:(ms.N_c), reac in ms.reaction[i_reacts]]...,
        # Enthalpy of reaction
        [cv.ΔHᵣ ~ reac.r(cv.p, cv.T, collect(cv.xᵢ[1, :])) * reac.Δhᵣ(cv.T) * cv.n
         for reac in ms.reaction[i_reacts]]...
    ]
    if flowtype == "const. mass"
        push!(eqs,
            0.0 ~ cv.c1.n * sum(ms.Mw[i] * cv.c1.xᵢ[i] for i in 1:(ms.N_c)) +
