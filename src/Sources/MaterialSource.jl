@component function MaterialSource(;
        substances_user,
        Nc = size(substances_user, 1),
        model,
        properties = Dict(
            subs => load_component_properties(subs)
    ...
    molar_to_mass_2 = [
        scalarize(Fᵂⱼᵢ[:, i] .~ (MWs[i] * gramsToKilograms) * Fⱼᵢ[:, i])
            for i in 1:Nc
    ]
    ...
    if T_user ≥ Tci
        pc = [
            ...
            P_dew ~ Pdew
            Hⱼ[1] ~
                enthalpy(model, P_user, T_user, zₜ_user, phase = :vapor) +
                sum(zₜ_user .* ΔH₀f)
            Hⱼ[2] ~
                enthalpy(model, P_user, T_user, zₜ_user, phase = :vapor) +
                sum(zₜ_user .* ΔH₀f)
        ...
        elseif P_user > Pbuble
            pc = [
                ...
                P_dew ~ Pdew
                Hⱼ[1] ~
                    enthalpy(model, P_user, T_user, zₜ_user, phase = :liquid) +
                    sum(zₜ_user .* ΔH₀f)
                Hⱼ[3] ~
                    enthalpy(model, P_user, T_user, zₜ_user, phase = :liquid) +
                    sum(zₜ_user .* ΔH₀f)
            ...
            xᵢⱼ, nᵢⱼ,
                G = tp_flash(model, P_user, T_user, zₜ_user, DETPFlash(; equilibrium = :vle)) #Phase i, component j
            ...
            pc = [
                α_g ~ sum(nᵢⱼ[2, :]) / (sum(nᵢⱼ[1, :]) + sum(nᵢⱼ[2, :])) # Vapor phase is the second entry
                Tc ~ Tci
                Pc ~ Pci
                P_buble ~ Pbuble
                P_dew ~ Pdew
                ...
                ρʷ[1] ~
                    (
                    sum(nᵢⱼ[2, :] .* MWs) * gramsToKilograms +
                        sum(nᵢⱼ[1, :] .* MWs) * gramsToKilograms
                ) / (V_l + V_g)
